<div class="train_visualizer">
    <% loop $SortedTrains() %>
    <div class="train <% if $Up.SortedTrains.Count <= 1 %>active<% end_if %> <% if $Children.GroupedBy('Wagon').Count < 2 %>no_wagons<% end_if %>" data-train="$Train" data-type="train">
            ...
            <% loop $Children.GroupedBy("Wagon") %>
                <div class="wagon">
                    ...
                    <% loop $Children.GroupedBy("Row") %>
                        <div class="row">
                        <% if $Up.Children.GroupedBy("Seat").Count > 1 %><p>$Row</p><% end_if %>
                            <% loop $Children.GroupedBy("Seat") %>
                                <div class="seat" data-behaviour="seat_selector" data-train="$Up.Up.Up.Up.Up.Up.Train" data-wagon="$Up.Up.Up.Up.Wagon" data-row="$Up.Up.Row" data-seat="$Seat">
                                    <% if $Up.Children.GroupedBy("Seat").Count > 1 %> <p>$Seat</p><% end_if %>
                                    <% if $Up.Children.GroupedBy("Seat").Count < 2 %> <p>$Up.Up.Row</p><% end_if %>
